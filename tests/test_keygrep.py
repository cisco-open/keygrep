...
def test_identical_keys(tmp_path):
    """Check that public/private keys with the same fingerprint are treated as the same."""
...
def test_openssh_keys(tmp_path):
    ...
    for key in priv_data:
        ...
        # For PEM/PKCS8 formatted private keys, we cannot obtain the fingerprint or public key without the passphrase
        else:
