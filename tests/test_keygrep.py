...
def test_empty_files(tmp_path: Path) -> None:
    ...
    kc.load_private_keys(Path(__file__).parent / "test-keys/empty_file")
    kc.load_public_keys(Path(__file__).parent / "test-keys/empty_file")

def test_comment_decoding(tmp_path: Path) -> None:
    """Test handling of keys with invalid UTF-8 in comment strings. Note that
    we don't check the public key comment, as comment strings cannot be
    unambiguously identified in OpenSSH formatted public keys in unstructured
    data."""

    kc = KeyChain(output_dir=tmp_path)
    kc.load_private_keys(Path(__file__).parent / "test-keys/rsa_1_invalid_unicode_comment")
    kc.load_public_keys(Path(__file__).parent / "test-keys/rsa_1_invalid_unicode_comment.pub")
    assert kc.private_keys[0]["sha256"] == "SHA256:l6itGumSMcRBBAFteCgmjQBIXqLK/jFGUH3viHX1RmE"
    assert kc.public_keys[0]["sha256"] == "SHA256:l6itGumSMcRBBAFteCgmjQBIXqLK/jFGUH3viHX1RmE"
    assert kc.private_keys[0]["comments"][0] == "\\364\\220\\200\\200"
...
def test_environment(tmp_path: Path) -> None:
    """Test that keys are detected under environment variable style mangling."""
    kc = KeyChain(output_dir=tmp_path, path_prefix=Path(__file__).parent)
    kc.load_private_keys(Path(__file__).parent / "test-keys/env")
    assert kc.private_keys[0]["sha256"] == "SHA256:l6itGumSMcRBBAFteCgmjQBIXqLK/jFGUH3viHX1RmE"
...
def test_unmangling(tmp_path: Path) -> None:
    ...
    assert list(kc.private_keys[0]["privkey_locations"].values()) == [[72, 494, 952]]
...
def test_invalid_public_keys(tmp_path: Path) -> None:
    ...
    kc = KeyChain(output_dir=tmp_path, include_mangled=True)
...
def test_openssh_keys(tmp_path: Path) -> None:
    ...
    kc = KeyChain(output_dir=tmp_path, include_mangled=False)
