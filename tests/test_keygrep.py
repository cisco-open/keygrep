...
def test_binary_comment(tmp_path: Path) -> None:
    """Test handling of keys with invalid UTF-8 in comment strings."""
    # Generating the test case key:
    # cp openssh/rsa_1 rsa_1_binary_comment
    # ssh-keygen -C "$(perl -e 'print pack "C*", 1..255')" -c -f rsa_1_binary_comment
    kc = KeyChain(output_dir=tmp_path, path_prefix=Path(__file__).parent, include_mangled=False)
    kc.load_private_keys(Path(__file__).parent / "test-keys/rsa_1_binary_comment")
    kc.load_public_keys(Path(__file__).parent / "test-keys/rsa_1_binary_comment.pub")
    assert kc.private_keys[0]["sha256"] == "SHA256:l6itGumSMcRBBAFteCgmjQBIXqLK/jFGUH3viHX1RmE"
    assert kc.private_keys[0]["comments"][0].startswith("\\001\\002\\003")

    # Public key has a truncated comment string (\x01 to \x07) due to whitespace linting.
    # Note that we don't check the public key comment, as comment strings
    # cannot be unambiguously identified in OpenSSH formatted public keys in
    # unstructured data.
